...
var Hashtag = artifacts.require("HashtagAsymDeal");
var HashtagList = artifacts.require("HashtagList");
const ipfs = require("nano-ipfs-store").at("https://ipfs.infura.io:5001");
const uuid = require("uuid");

contract('HashtagAsymDeal', (accounts) => {
    ...
    describe('Happy Flow: Item Update Stage', function () {
        it("should add funds to the new Item on the Hashtag contract", async function () {
            var itemOnContract = await hashtagContract.readItem(currentItemHash);
            var tempItem = await ipfs.cat(itemOnContract[6]); 
            var itemMetaJson_old = JSON.parse(tempItem);

            var itemMetaJson = {
                "itemHash": itemMetaJson_old.itemHash,
                "username": "Frank",
                "avatarHash": "QmSwyxpLq1h8gJe4uSRXgyStfMSonZTKcFAL6yuPB2QLEh",
                "address": seeker, 
                "description": "Translate my 1 page manifesto from English to Dutch",
                "hashtagFee": itemMetaJson_old.hashtagFee,
                "itemValue": 40e18 
            };
            var itemMetaHash = await ipfs.add(JSON.stringify(itemMetaJson));

            var hashtagContractInstance = await web3.eth.contract(hashtagContract.abi).at(hashtagContract.address);
            var txdata = hashtagContractInstance.updatePlusItem.getData(itemMetaJson_old.itemHash, itemMetaJson.itemValue, itemMetaHash, {
                from: seeker,
            });
            var requestValue = itemMetaJson.itemValue - itemMetaJson_old.itemValue;
            var result = await swtToken.approveAndCall(hashtagContract.address, requestValue, txdata, {
                from: seeker,
                gas: 4700000
            });
        ...
        it("should see correct token balance Seeker account", async function () {
            var balance = await swtToken.balanceOf(seeker);
            assert.equal(balance.toNumber(), 59700000000000000000, "Seeker balance not correct");
        });

        it("should see correct token balance Hashtag account", async function () {
            var balance = await swtToken.balanceOf(hashtagContract.address);
            assert.equal(balance.toNumber(), 40000000000000000000, "Hashtag balance not correct");
        });
        ...
        it("should detract funds from the new Item on the Hashtag contract", async function () {
            var itemOnContract = await hashtagContract.readItem(currentItemHash);
            var tempItem = await ipfs.cat(itemOnContract[6]); 
            var itemMetaJson_old = JSON.parse(tempItem);
            ...
            var hashtagContractInstance = await web3.eth.contract(hashtagContract.abi).at(hashtagContract.address);
            var result = await hashtagContractInstance.updateMinItem(itemMetaJson_old.itemHash, itemMetaJson.itemValue, itemMetaHash, {
                from: seeker,
                gas: 4700000
            });
            assert.isNotNull(result);
